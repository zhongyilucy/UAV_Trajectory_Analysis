...
position_noise = posFake+np.random.normal(0,0.05,size=(t.shape[0]))
...
for i in range(1,t.shape[0]):#循环从1开始到样本数前一个结束
    ...
    dv =  (position[i]-position[i-1]) + np.random.normal(0,1) # 模拟从IMU读取出的速度
    #------------------预测--------------------
...
for x in np.arange(0, t.shape[0]):
    if predicts[x] - posFake[x]<-0.15 or predicts[x] - posFake[x] > 0.15:
        error.append(x)
